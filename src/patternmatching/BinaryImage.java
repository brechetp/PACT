...
public class BinaryImage {
	...
	public Pixel [][] conncetedComponents(){
		...
		for (int i=0; i<size; i++){
			for (int j=0; j<size; j++){
				if (taggedBinaryImage[i][j].getNorthNeighbor().getTag()==0 && taggedBinaryImage[i][j].getWestNeighbor().getTag()==0){
					taggedBinaryImage[i][j].setTag(k);	k++;
				} else {
					if (taggedBinaryImage[i][j].getNorthNeighbor().getTag()==taggedBinaryImage[i][j].getWestNeighbor().getTag()){
						taggedBinaryImage[i][j].setTag(taggedBinaryImage[i][j].getNorthNeighbor().getTag());
							} else {
								int e = min(taggedBinaryImage[i][j].getNorthNeighbor().getTag(),taggedBinaryImage[i][j].getWestNeighbor().getTag());
								if (e==0){
									e = max(taggedBinaryImage[i][j].getNorthNeighbor().getTag(),taggedBinaryImage[i][j].getWestNeighbor().getTag());	
								}
								taggedBinaryImage[i][j].setTag(connectionTable[e]);
								// mise à jour de la table de la table de correspondance
				...
				for (int i=0; i<size; i++){
					for (int j=0; j<size; j++){
						if (taggedBinaryImage[i][j].getTag() != 0) {
							taggedBinaryImage[i][j].setTag(connectionTable[taggedBinaryImage[i][j].getTag()]);
					}
				}
				}
